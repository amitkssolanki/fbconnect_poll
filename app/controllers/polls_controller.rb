class PollsController < ApplicationController
  ...
  # POST /polls.xml
  def create
    @poll = Poll.new(params[:poll].merge(:user_id => current_user.id))
  ...
  def vote
    @poll = Poll.find(params[:id])
    @poll.votes.create(:user_id => current_user.id, :status => params[:status])
    flash[:success] = "Your vote has been successfully recorded."
    respond_to do |format|
      format.html { redirect_to(@poll) }
      format.xml  { head :ok }
    end    
